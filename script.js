...
$('.lien-view').click(function(){
    $('div.view').slideToggle('slow');
});
...
var movies = [
...

	{
		...
		image: 'https://www.themoviedb.org/t/p/w300_and_h450_bestv2/wu361kPckigxzW19qtUbpCDzg0f.jpg',
...
// selection
     ...
     function verifyForm(event) {
		 var nbrErrors = 0;
        // couper le comportement par défaut qui recharge la page courante
        event.preventDefault();
     	var pseudo = $('input').val();
        var note = $('select').val();
        var avis = $('textarea').val();
        
        // traiter les Regles de validation
        // 1 input - min.2 caractères et max.7 caractères
        if(pseudo.length<2 || pseudo.length>7) {
        	// afficher un texte d'erreur
            // avec .next() je peux selectionner la balise suivante
			$('input').next().text('Minimum 2 caractères et maximum 7 caractères').show();
			nbrErrors++;    
        } else {
        	$('input').next().hide();
        }
         
        // forcer à donner une note
        if (note == "") {
			$('select').next().text('Choisir une note').show();
			nbrErrors++;
        } else {
            $('select').next().hide();
        }
        
        if (avis.length<10 || avis.length>100) {
			$('textarea').next().text('Minimum 10 caractères et max 100 carac.').show();
			nbrErrors++;
        } else {
            $('textarea').next().hide();
		}
		if(nbrErrors == 0) {
			$('div#success-message').text('Cool votre avis est envoyé')
			.addClass('alert-success d-block').removeClass('d-none'); // .show()
	   }
	   else {
		   $('div#success-message').addClass('d-none').removeClass('d-block'); // .hide()
	   }  
	}
    
        
        // 2 poser un ecouteur d'evt 'submit'
        form.on('submit', verifyForm);
...
//if(pseudo.lenght<2 || pseudo.lenght>7) {
  //  $('input').next().text('Minimum 2 caractères et max 7 caractères').show();
  //  } else 
//} 
//ecouteur
//form.on('submit' , verifyform);